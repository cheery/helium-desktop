...
fragment:
    // from: http://glsl.heroku.com/e#3572.3
    //some ugly clouds
    ...
    void main( void ) {
        ...
        float s =(-qx-qy+0.2);
        if (s<-0.05) s=-0.05;
        vec3 d=s*vec3(0.7,0.8,0.6);
        //d=max(vec3(0.0),d);
        //d+=0.1;
        d*=0.2;
        d=mix(vec3(0.2,0.3,0.6)*0.1+d*2.0,vec3(0.5,0.5,1.0)*0.8,1.0-pow(q,0.04)*1.1);
        d*=8.0;
        
        //Stars
        float scale = 8.0;
        vec2 position2 = ((((gl_FragCoord.xy / resolution )) - 0.4) * scale);
        float gradient = 0.0;
        float fade = 0.0;
        float z = 0.0;
        vec2 centered_coord = position2*rand(position)+s+1.0;
        vec3 color;
        for (float i=1.0; i<=60.0; i++)
        {
            vec2 star_pos = vec2(rand(vec2(i)) * 250.0, rand(vec2(i*i)) * 250.0);
            float z = mod(sin(i*i*i)*time + 200.0, 256.0);
